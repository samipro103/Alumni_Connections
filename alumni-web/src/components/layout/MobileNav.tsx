...
import { House, Search, Plus, MessageCircle, Settings } from "lucide-react";
...
const items = [
  ...
  { href: "/messages", label: "Mensajes", icon: MessageCircle, messages: true },
...
export default function MobileNav() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`mobile-msg:${currentUser.id}`)
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-white/[0.08] bg-[#0b0e13]/95 px-2 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl lg:hidden">
      <div className="mx-auto grid max-w-lg grid-cols-5">
        {items.map(({ href, label, icon: Icon, create, messages }) => {
          const active =
            !create &&
            (pathname === href ||

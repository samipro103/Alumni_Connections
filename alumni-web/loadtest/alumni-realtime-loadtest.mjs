...
const MAX_INFLIGHT = numericArg("maxInflight", 40, 1, 200);
...
let peakInflightWrites = 0;

const runNonce = BigInt(Date.now() % 100000000000);
...
console.log("");
console.log("ALUMNI 3.7.8G - PACED REALTIME LOAD TEST");
console.log("-----------------------------------------");
console.log(`Target: ${BASE_URL}`);
console.log(`Connections: ${CONNECTIONS}`);
console.log(`Ramp: ${RAMP_SECONDS}s`);
console.log(`Duration: ${DURATION_SECONDS}s`);
console.log(`Target event rate: ${EVENTS_PER_SECOND}/s`);
console.log(`Max concurrent writes: ${MAX_INFLIGHT}`);
console.log(`Settle after writes: ${SETTLE_SECONDS}s`);
console.log("Realtime table: lt_messages only");
...
for (let i = 0; i < CONNECTIONS; i += 1) {
  ...
  const receiverId = syntheticUuid(
    "alumni-lt-user-",
    userNo
  );
...
if (connected.size < CONNECTIONS) {
  console.log(
    `WARNING: ${connected.size}/${CONNECTIONS} conexiones SUBSCRIBED.`
  );
}

const writer = makeClient();
const targetEvents = Math.floor(
  DURATION_SECONDS * EVENTS_PER_SECOND
);
const intervalMs = 1000 / EVENTS_PER_SECOND;
const inflight = new Set();
const allWrites = [];

async function sendOne(sequence) {
  const targetIndex =
    (sequence - 1) % CONNECTIONS;

  const receiverNo =
    (targetIndex % 4000) + 1;

  const senderNo =
    ((receiverNo + 137 - 1) % 4000) + 1;
  ...
  const { error } = await writer
    ...
    .insert({
      ...
      content: `LT: realtime paced seq=${sequence}`,
    });
...
for (
  let sequence = 1;
  sequence <= targetEvents;
  sequence += 1
) {
  const scheduledAt =
    dispatchStarted +
    (sequence - 1) * intervalMs;
  ...
  const promise = sendOne(sequence);

  inflight.add(promise);
  allWrites.push(promise);

  peakInflightWrites = Math.max(
    peakInflightWrites,
    inflight.size
  );
...
const dispatchElapsedMs =
  performance.now() - dispatchStarted;

await Promise.allSettled(allWrites);

const writesCompletedAt = performance.now();
const writeCompletionElapsedMs =
  writesCompletedAt - dispatchStarted;

await sleep(SETTLE_SECONDS * 1000);

const finalConnected = connected.size;
const missingEvents = Math.max(
  0,
  eventsInserted - eventsReceived
);
...
const actualAttemptedEps =
  dispatchElapsedMs > 0
    ? eventsAttempted /
      (dispatchElapsedMs / 1000)
    : 0;

const actualInsertedEps =
  writeCompletionElapsedMs > 0
    ? eventsInserted /
      (writeCompletionElapsedMs / 1000)
    : 0;
...
const summary = {
  ...
  connectElapsedMs: Number(
    connectElapsedMs.toFixed(2)
  ),
  rampSeconds: RAMP_SECONDS,
  durationSeconds: DURATION_SECONDS,
  targetEventsPerSecond: EVENTS_PER_SECOND,
  targetEvents,
  eventsAttempted,
  eventsInserted,
  eventsReceived,
  missingEvents,
  deliveryPct: Number(
    deliveryPct.toFixed(3)
  ),
  actualRates: {
    attemptedPerSecond: Number(
      actualAttemptedEps.toFixed(2)
    ),
    insertedPerSecond: Number(
      actualInsertedEps.toFixed(2)
    ),
    dispatchElapsedSeconds: Number(
      (dispatchElapsedMs / 1000).toFixed(2)
    ),
    writeCompletionElapsedSeconds: Number(
      (
        writeCompletionElapsedMs / 1000
      ).toFixed(2)
    ),
  ...
  concurrency: {
    maxInflightWrites: MAX_INFLIGHT,
    peakInflightWrites,
  },
  reconnectSignals,
  latencyMs: {
  ...
  subscriptionErrors:
    subscriptionErrors.slice(0, 25),
  sendErrors:
    sendErrors.slice(0, 25),
};
...
const output =
  `loadtest/results/realtime-paced-${CONNECTIONS}conn-${EVENTS_PER_SECOND}eps-${stamp}.json`;
...
if (
  ...
  deliveryPct >= 99 &&
  actualAttemptedEps >=
    EVENTS_PER_SECOND * 0.95
) {
  console.log(
    "RESULTADO: Realtime estable en la tasa objetivo."
  );
} else {
  console.log(
    "RESULTADO: revisar antes de aumentar conexiones o EPS."
  );
}

...
const MARKER = "ALUMNI_PROFILE_1_2_0B_LAUNCH_MATCH_OPTION_3";
...
if (
  ...
  console.log("ℹ️ Profile 1.2.0B ya está aplicado.");
  process.exit(0);
}
...
/* Añadir TODOS los iconos de una sola vez, sin depender del orden actual. */
...
const lucideImportMatch = source.match(
...
const currentLucideNames = lucideImportMatch[1]
  .split(",")
  .map((name) => name.trim())
  .filter(Boolean);

const nextLucideNames = [
  ...currentLucideNames,
  ...lucideAdditions.filter(
    (name) => !currentLucideNames.includes(name)
  ),
];

const nextLucideImport = `import {\n${nextLucideNames
  .map((name) => `  ${name},`)
  .join("\n")}\n} from "lucide-react";`;
...
console.log("✅ Iconos lucide-react agregados de forma robusta");
...
for (const token of requiredProfileTokens) {
  if (!source.includes(token)) {
    fail("Validación Profile 1.2.0B: falta " + token);
  }
}
...
const profileBackup =
  PROFILE + ".before-profile-1.2.0b.bak";

const socialBackup =
  SOCIALS + ".before-profile-1.2.0b.bak";
...
console.log("");
console.log("✅ ALUMNI Profile 1.2.0B — Launch Match aplicado.");
